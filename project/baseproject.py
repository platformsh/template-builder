...
class BaseProject(object):
    ...
    def init(self):
        ...
        return [
            "git clone git@github.com:platformsh-templates/{0}.git {1}".format(
                name, self.builddir
            )
        ]
    ...
    def platformify(self):
        ...
        for patch in patches:
            actions.append("cd {0} && patch -p1 < {1}".format(self.builddir, patch))
    ...
    def package_update_actions(self):
        ...
        for file, command in self.updateCommands.items():
            actions.append(
                'cd {0} && [ -f {1} ] && {2} || echo "No {1} file found, skipping."'.format(
                    self.builddir, file, command
                )
            )
