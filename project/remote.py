...
class RemoteProject(BaseProject):
    ...
    def update(self):
        ...
        if hasattr(self, 'tag'):
            actions.append('cd {0} && git merge --allow-unrelated-histories -X theirs --squash {1}'.format(
                self.builddir, self.tag)
            )
        elif hasattr(self, 'update_branch'):
            actions.append('cd {0} && git merge --allow-unrelated-histories -X theirs --squash project/{1}'.format(
